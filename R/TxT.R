...
#'  @export
#--------------------------------------------------------------------------------------
TxT<<- function(tp,fp,fn,tn,do.p=TRUE,chemclass=NA,signature=NA, gene_target=NA) {
  sens <- tp/(tp+fn)
  spec <- tn/(tn+fp)
  ppv  <- tp/(tp+fp)
  npv  <- tn/(tn+fn)
  ...
  if(!is.na(gene_target)) name.list <- c("gene_target",name.list)
  if(!is.na(signature)) name.list <- c("signature",name.list)
  if(!is.na(chemclass)) name.list <- c("chemclass",name.list)
  ...
  if(!is.na(signature)) mat[1,"gene_target"] <- gene_target
  if(!is.na(signature)) mat[1,"signature"] <- signature
  if(!is.na(chemclass)) mat[1,"chemclass"] <- chemclass
